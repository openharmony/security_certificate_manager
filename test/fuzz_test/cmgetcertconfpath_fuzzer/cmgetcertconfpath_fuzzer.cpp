...
#include "cmgetcertconfpath_fuzzer.h"
...
namespace OHOS {
    bool DoSomethingInterestingWithMyAPI(const uint8_t* data, size_t size)
    {
        ...
        if (!GetUintFromBuffer(myData, &remainSize, &offset, &confFilePathLen)) {
            CmFree(myData);
            return false;
        }
        ...
        char confFilePath[1024] = "confFilePath";
        CertmanagerTest::MockHapToken mockHap;
        (void)CmGetCertConfPath(userId, uid, &certUri, confFilePath, confFilePathLen);
        CmFree(myData);
        return true;
    }
}
