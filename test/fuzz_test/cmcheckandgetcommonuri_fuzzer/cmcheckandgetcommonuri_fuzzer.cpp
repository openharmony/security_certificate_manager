...
#include "cmcheckandgetcommonuri_fuzzer.h"
...
namespace OHOS {
    bool DoSomethingInterestingWithMyAPI(const uint8_t* data, size_t size)
    {
        ...
        struct CmBlob commonUri = {0, nullptr};
        if (!GetCmBlobFromBuffer(myData, &remainSize, &offset, &commonUri)) {
            CmFree(myData);
            return false;
        }

        CertmanagerTest::MockHapToken mockHap;
        (void)CmCheckAndGetCommonUri(&cmContext, store, &keyUri, &commonUri);
        (void)CmAuthDeleteAuthInfoByUid(userId, targetUid, &keyUri);
        CmFree(myData);
        return true;
    }
}
