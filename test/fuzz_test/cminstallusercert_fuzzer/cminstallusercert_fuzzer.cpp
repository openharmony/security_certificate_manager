...
#include "cminstallusercert_fuzzer.h"
...
namespace OHOS {
    bool DoSomethingInterestingWithMyAPI(const uint8_t* data, size_t size)
    {
        ...
        (void)CmInstallUserCert(&cmContext, &userCert, &certAlias, status, &certUri);
        (void)CmInstallMultiUserCert(&cmContext, &userCert, &certAlias, status, &certUri);
        (void)CmUninstallUserCert(&cmContext, &certUri);
        (void)CmUninstallAllUserCert(&cmContext);
        (void)CmSetStatusBackupCert(&cmContext, &certUri, store, status);
        CmFree(myData);
        return true;
    }
}
