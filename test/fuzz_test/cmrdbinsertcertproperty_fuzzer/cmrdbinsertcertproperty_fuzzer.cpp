...
#include "cmrdbinsertcertproperty_fuzzer.h"
...
namespace OHOS {
    bool DoSomethingInterestingWithMyAPI(const uint8_t* data, size_t size)
    {
        ...
        if (!GetCmBlobFromBuffer(myData, &remainSize, &offset, &subjectName)) {
            CmFree(myData);
            return false;
        }
        ...
        struct CertPropertyOri certPropertyOri;
        certPropertyOri.context = &cmContext;
        certPropertyOri.uri = &uri;
        certPropertyOri.alias = &alias;
        certPropertyOri.subjectName = &subjectName;
        certPropertyOri.store = store;
        CertmanagerTest::MockHapToken mockHap;
        (void)RdbInsertCertProperty(&certPropertyOri);
        CmFree(myData);
        return true;
    }
}
