...
#include "cmgetnameencode_fuzzer.h"
...
namespace OHOS {
    bool DoSomethingInterestingWithMyAPI(const uint8_t* data, size_t size)
    {
        ...
        uint32_t offset = 0;
        struct CmBlob inBlob = {0, nullptr};
        if (!GetCmBlobFromBuffer(myData, &remainSize, &offset, &inBlob)) {
            CmFree(myData);
            return false;
        }
        ...
        if (outBlob.size < OUT_BLOB_SIZE) {
            CmFree(myData);
            return false;
        }

        CertmanagerTest::MockHapToken mockHap;
        (void)GetNameEncode(&inBlob, &outBlob);
        CmFree(myData);
        return true;
    }
}
