...
#include "cmsetcertstatus_fuzzer.h"
...
namespace OHOS {
    ...
    {
        uint32_t minSize = sizeof(struct CmBlob) + sizeof(uint32_t) + sizeof(bool);
        ...
        struct CmBlob sysCertUri = { 0, nullptr };
        if (!GetCmBlobFromBuffer(myData, &remainSize, &offset, &sysCertUri)) {
            CmFree(myData);
            return false;
        }
