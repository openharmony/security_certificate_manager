...
#include "cmgetcertbackupdir_fuzzer.h"
...
namespace OHOS {
    bool DoSomethingInterestingWithMyAPI(const uint8_t* data, size_t size)
    {
        ...
        uint32_t certBackupDirLen;
        if (!GetUintFromBuffer(myData, &remainSize, &offset, &certBackupDirLen)) {
            CmFree(myData);
            return false;
        }

        char cerBackupDir[1024] = "certBackupDir";
        CertmanagerTest::MockHapToken mockHap;
        (void)CmGetCertBackupDir(userId, cerBackupDir, certBackupDirLen);
        CmFree(myData);
        return true;
    }
}
