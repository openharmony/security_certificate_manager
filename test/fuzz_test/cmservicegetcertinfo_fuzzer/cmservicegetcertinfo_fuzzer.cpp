...
#include "cmservicegetcertinfo_fuzzer.h"
...
namespace OHOS {
    bool DoSomethingInterestingWithMyAPI(const uint8_t* data, size_t size)
    {
        ...
        if (!GetCmBlobFromBuffer(myData, &remainSize, &offset, &certificateData)) {
            CmFree(myData);
            return false;
        }
        ...
        (void)CmServiceGetCertInfo(&cmContext, &certUri, store, &certificateData, &status);
        CmFree(myData);
        return true;
    }
}
