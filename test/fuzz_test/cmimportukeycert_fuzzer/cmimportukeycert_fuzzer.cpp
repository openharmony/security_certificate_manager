...
#include "cmimportukeycert_fuzzer.h"
...
{
    uint32_t minSize = sizeof(struct CmBlob) * 2 + sizeof(struct UkeyInfo);
    ...
    struct CmBlob cert = { 0, nullptr };
    if (!GetCmBlobFromBuffer(myData, &remainSize, &offset, &cert)) {
        CmFree(myData);
        return false;
    }
    ...
    (void)CmImportUkeyCert(&keyUri, &cert, &ukeyInfo);
