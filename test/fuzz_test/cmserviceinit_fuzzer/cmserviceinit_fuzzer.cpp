...
#include "cmserviceinit_fuzzer.h"
...
namespace OHOS {
    static bool CreateCmSignatureSpec(struct CmSignatureSpec &cmSignatureSpec,
        uint8_t *myData, uint32_t &remainSize, uint32_t &offset)
    {
        uint32_t purpose;
        ...
        cmSignatureSpec.purpose = purpose;
        cmSignatureSpec.digest = digest;
        cmSignatureSpec.padding = padding;
        return true;
    }
    ...
    bool DoSomethingInterestingWithMyAPI(const uint8_t* data, size_t size)
    {
        ...
        struct CmSignatureSpec signatureSpec;
        if (!CreateCmSignatureSpec(signatureSpec, myData, remainSize, offset)) {
            CmFree(myData);
            return false;
        }

        CertmanagerTest::MockHapToken mockHap;
        (void)CmServiceInit(&cmContext, &authUri, &signatureSpec, &handle);
        (void)CmServiceUpdate(&cmContext, &handle, &inData);
        (void)CmServiceFinish(&cmContext, &handle, &inData, &outData);
        (void)CmServiceAbort(&cmContext, &handle);
        CmFree(myData);
        return true;
    }
}
