...
#include "cmreadcertdata_fuzzer.h"
...
namespace OHOS {
    bool DoSomethingInterestingWithMyAPI(const uint8_t* data, size_t size)
    {
        ...
        struct CmBlob userCertData = {0, nullptr};
        if (!GetCmBlobFromBuffer(myData, &remainSize, &offset, &userCertData)) {
            CmFree(myData);
            return false;
        }
        ...
        (void)CmReadCertData(store, &cmContext, &certUri, &userCertData);
        (void)CmBackupUserCert(&cmContext, &certUri, &certData);
