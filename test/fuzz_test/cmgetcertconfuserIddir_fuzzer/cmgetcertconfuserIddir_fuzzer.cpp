...
#include "cmgetcertconfuserIddir_fuzzer.h"
...
namespace OHOS {
    bool DoSomethingInterestingWithMyAPI(const uint8_t* data, size_t size)
    {
        ...
        uint32_t dirLen;
        if (!GetUintFromBuffer(myData, &remainSize, &offset, &dirLen)) {
            CmFree(myData);
            return false;
        }
        ...
        char confUserIdDir[1024] = "confUserIdDir";
        CertmanagerTest::MockHapToken mockHap;
        (void)CmGetCertConfUserIdDir(userId, confUserIdDir, dirLen);
        (void)CmGetCertConfUidDir(userId, uid, confUserIdDir, dirLen);
        CmFree(myData);
        return true;
    }
}
