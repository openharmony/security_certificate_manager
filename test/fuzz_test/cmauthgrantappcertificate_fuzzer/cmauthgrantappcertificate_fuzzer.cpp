...
#include "cmauthgrantappcertificate_fuzzer.h"
...
namespace OHOS {
    bool DoSomethingInterestingWithMyAPI(const uint8_t* data, size_t size)
    {
        ...
        cmAppUidList.appUid = &userId;
        CmAuthStorageLevel cmAuthStorageLevel = CM_AUTH_STORAGE_LEVEL_EL1;
        CertmanagerTest::MockHapToken mockHap;
        (void)CmAuthGrantAppCertificate(&cmContext, &keyUri, userId, &authUri);
        (void)CmAuthGetAuthorizedAppList(&cmContext, &keyUri, &cmAppUidList);
        (void)CmAuthIsAuthorizedApp(&cmContext, &authUri);
        (void)CmAuthRemoveGrantedApp(&cmContext, &keyUri, userId);
        (void)CmAuthDeleteAuthInfo(&cmContext, &keyUri, cmAuthStorageLevel);
        (void)CmAuthDeleteAuthInfoByUserId(userId, &keyUri);
        (void)CmCheckCallerIsProducer(&cmContext, &keyUri);
        CmFree(myData);
        return true;
    }
}
