...
/**
 * @tc.name: SysAppCertTest046
 * @tc.desc: Test Install sys app cert chain and priv key normal function
 * @tc.type: FUNC
 */
HWTEST_F(CmSysAppCertTest, SysAppCertTest046, TestSize.Level0)
{
    char retUriBuf[MAX_LEN_URI] = {0};
    struct CmBlob sysKeyUri = { sizeof(retUriBuf), reinterpret_cast<uint8_t *>(retUriBuf) };
    ...
    struct CmAppCertParam appCertParam = { (struct CmBlob *)&g_appPemCertChain, &credPwd, &certAlias,
        CM_SYS_CREDENTIAL_STORE, TEST_USERID, CM_AUTH_STORAGE_LEVEL_EL1, CHAIN_KEY,
        (struct CmBlob *)&g_appPemCertPrivKey, SHA256_FORMAT };
    int32_t ret = CmInstallAppCertEx(&appCertParam, &sysKeyUri);
    EXPECT_EQ(ret, CM_SUCCESS) << "SysAppCertTest046 test install pem failed, retcode:" << ret;

    ret = CmUninstallAppCert(&sysKeyUri, CM_SYS_CREDENTIAL_STORE);
    EXPECT_EQ(ret, CM_SUCCESS) << "SysAppCertTest046 test uninstall failed, retcode:" << ret;
}

/**
 * @tc.name: SysAppCertTest047
 * @tc.desc: Test Install sys app cert chain and priv key normal function
 * @tc.type: FUNC
 */
HWTEST_F(CmSysAppCertTest, SysAppCertTest047, TestSize.Level0)
{
    char retUriBuf[MAX_LEN_URI] = {0};
    struct CmBlob sysKeyUri = { sizeof(retUriBuf), reinterpret_cast<uint8_t *>(retUriBuf) };
    struct CmBlob credPwd = { 0, NULL };
    ...
    int32_t ret = CmInstallAppCertEx(&appCertParam, &sysKeyUri);
    EXPECT_EQ(ret, CM_SUCCESS) << "SysAppCertTest047 test install der failed, retcode:" << ret;

    ret = CmUninstallAppCert(&sysKeyUri, CM_SYS_CREDENTIAL_STORE);
    EXPECT_EQ(ret, CM_SUCCESS) << "SysAppCertTest047 test uninstall failed, retcode:" << ret;
}
}
