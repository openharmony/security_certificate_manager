...
namespace CMNapi {
CmUIExtensionStringCallback::CmUIExtensionStringCallback(
    std::shared_ptr<CmUIExtensionRequestContext>& reqContext) : CmUIExtensionCallback(reqContext) {}

CmUIExtensionStringCallback::~CmUIExtensionStringCallback()
{
    CM_LOG_D("~CmUIExtensionStringCallback");
}

void CmUIExtensionStringCallback::OnReceive(const OHOS::AAFwk::WantParams& request)
{
    CM_LOG_D("CmUIExtensionStringCallback OnReceive()");
...
void CmUIExtensionStringCallback::ProcessCallback(napi_env env, const CommonAsyncContext* asyncContext)
{
