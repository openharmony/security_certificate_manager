...
constexpr int CM_NAPI_GET_APP_CERT_INFO_ARGS_CALLBACK = 2;
}  // namespace
...
{
    size_t argc = CM_NAPI_GET_APP_CERT_INFO_ARGS_CALLBACK;
    napi_value argv[CM_NAPI_GET_APP_CERT_INFO_ARGS_CALLBACK] = { nullptr };
...
napi_value CMNapiGetAppCertInfoCommon(napi_env env, napi_callback_info info, uint32_t store)
{
    ...
    const char *jsName = "getAppCertificateInfo";
    if (store == APPLICATION_CERTIFICATE_STORE) {
        jsName = "getPublicCertificate";
    } else if (store == APPLICATION_PRIVATE_CERTIFICATE_STORE) {
        jsName = "getPrivateCertificate";
