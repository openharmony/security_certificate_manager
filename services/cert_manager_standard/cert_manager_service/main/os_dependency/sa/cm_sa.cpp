...
static int32_t GetSrcData(MessageParcel &data, struct CmBlob *srcData)
{
    ...
    const uint8_t *pdata = bufferData == nullptr ?
        static_cast<const uint8_t *>(data.ReadRawData(static_cast<size_t>(srcData->size))) : bufferData;
    if (pdata == nullptr) {
...
void CertManagerService::OnStart(const SystemAbilityOnDemandReason& startReason)
{
    ...
    if (startReason.GetId() == OnDemandReasonId::COMMON_EVENT &&
        ...
        context.userId = static_cast<uint32_t>(startReason.GetExtraData().GetCode());
